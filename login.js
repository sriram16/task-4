...
function register() {
    ...
    if (!username) {
        registerErrorMessage.textContent = 'Please enter your Gmail';
    } else if (!isValidPassword(password)) {
        registerPasswordErrorMessage.textContent = 'Please enter an alphanumeric password of at least 8 characters';
    } else if (!isUsernameAvailable(username)) {
        registerErrorMessage.textContent = 'Gmail already exists. Please choose a different Gmail';
    } else {
...
function login() {
    ...
    if (localStorage.getItem(username) === password) {
        ...
        loginErrorMessageEl.textContent = 'Please enter your Gmail';  
    } else if (!isValidPassword(password)) {
        loginPasswordErrorMessageEl.textContent = 'Please enter an alphanumeric password of at least 8 characters';
        alreadyExistErrorMessageEl.textContent = '';
    } else {
        alreadyExistErrorMessageEl.textContent = 'Invalid Gmail or Password';
    }
